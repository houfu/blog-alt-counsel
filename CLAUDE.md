...
| `workflow-gate.js` | PreToolUse (Task) | **Denies** reviewer/audit spawns that break workflow rules: round caps (2 reviewer / 1 audit per draft, counted by draft content hash in `posts/<folder>/.workflow.json`), missing pitch checkpoint for the current draft, missing reviewer memory file, and silent persona fallback (general-purpose + personas brief needs `ACKNOWLEDGED-FALLBACK`). Manage state via `scripts/workflow-state.js` (checkpoint / length-audit / override / show) |
